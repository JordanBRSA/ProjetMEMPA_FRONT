...
const seConnecter = async (req, res) => {
    ...
    try {
        const result = await utilisateur.findOne({
            where: { nom_util: login, mot_de_passe: pass }
        });
        ...
        const token = jwt.sign({ id: result.id_util, login }, process.env.JWT_TOKEN, {expiresIn: '1d'});
...
const creerCompte = async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ id: nouvCompte.id_util, login }, process.env.JWT_TOKEN, {expiresIn: '1d'});
